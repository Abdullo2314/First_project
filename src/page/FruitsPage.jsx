...
function FruitsPage() {
    ...
    useEffect(() => {
        axios.get("http://localhost:8000/api")
            .then((res) => {
                const fruitProducts = res.data.filter(product => product.category === "fruits");
    ...
    if (loading) {
        return <div className="text-center mt-10">Загрузка фруктов...</div>;
    }

    return (
        <div className="container max-w-[1140px] mx-auto p-6">
            <h1 className="text-3xl font-bold mb-6"> Фрукты</h1>
            {products.length === 0 ? (
